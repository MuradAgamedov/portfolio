...
class OptimizeServiceIcons extends Command
{
    protected $signature = 'images:optimize-service-icons {--webp : Convert images to WebP format}';
    protected $description = 'Optimize service icons - resize to 200x200 and compress using native PHP. Use --webp to convert to WebP format';

    public function handle()
    {
        $convertToWebp = $this->option('webp');
        
        $this->info('Starting service icons optimization...');
        if ($convertToWebp) {
            $this->info('WebP conversion enabled');
        }
        
        $serviceIconsPath = 'services/icons';
        $files = Storage::disk('public')->files($serviceIconsPath);
    ...
    private function optimizeImage($filePath, $convertToWebp = false)
    {
        ...
        if ($saved) {
            ...
            if ($convertToWebp && function_exists('imagewebp')) {
                ...
                $this->updateServiceIconInDatabase($filePath, $webpPath);
            }
    ...
    private function updateServiceIconInDatabase($originalPath, $webpPath)
    {
        try {
            // Find service with this icon (try both original and WebP paths)
            $service = Service::where('icon', $originalPath)
                ->orWhere('icon', $webpPath)
                ->first();
            
            if ($service) {
                // Check if already using WebP
                if ($service->icon === $webpPath) {
                    $this->line("Service ID {$service->id} already uses WebP icon: {$webpPath}");
                } else {
                    // Update to WebP version
                    $service->update(['icon' => $webpPath]);
                    $this->line("Database updated: Service ID {$service->id} now uses WebP icon");
                ...
                $this->warn("No service found with icon: {$originalPath}");
            }
        } catch (\Exception $e) {
