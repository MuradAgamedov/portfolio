...
class OptimizeBlogCardImages extends Command
{
    protected $signature = 'images:optimize-blog-cards {--webp : Convert images to WebP format}';
    protected $description = 'Optimize blog card images - resize to 350x250 and compress using native PHP. Use --webp to convert to WebP format';

    public function handle()
    {
        $convertToWebp = $this->option('webp');
        
        $this->info('Starting blog card images optimization...');
        if ($convertToWebp) {
            $this->info('WebP conversion enabled');
        }
        
        $blogCardPath = 'blogs/card';
        $files = Storage::disk('public')->files($blogCardPath);
    ...
    private function optimizeImage($filePath, $convertToWebp = false)
    {
        ...
        if ($saved) {
            ...
            if ($convertToWebp && function_exists('imagewebp')) {
                ...
                $this->updateBlogCardImageInDatabase($filePath, $webpPath);
            }
    ...
    private function updateBlogCardImageInDatabase($originalPath, $webpPath)
    {
        try {
            // Find blog with this card image (try both original and WebP paths)
            $blog = Blog::where('card_image', $originalPath)
                ->orWhere('card_image', $webpPath)
                ->first();
            
            if ($blog) {
                // Check if already using WebP
                if ($blog->card_image === $webpPath) {
                    $this->line("Blog ID {$blog->id} already uses WebP card image: {$webpPath}");
                } else {
                    // Update to WebP version
                    $blog->update(['card_image' => $webpPath]);
                    $this->line("Database updated: Blog ID {$blog->id} now uses WebP card image");
                ...
                $this->warn("No blog found with card image: {$originalPath}");
            }
        } catch (\Exception $e) {
