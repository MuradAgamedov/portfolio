...
class OptimizeCertificateImages extends Command
{
    protected $signature = 'images:optimize-certificates {--webp : Convert images to WebP format}';
    protected $description = 'Optimize certificate images - resize to 350x250 and compress using native PHP. Use --webp to convert to WebP format';

    public function handle()
    {
        $convertToWebp = $this->option('webp');
        
        $this->info('Starting certificate images optimization...');
        if ($convertToWebp) {
            $this->info('WebP conversion enabled');
        }
        
        $certificatePath = 'certificates/images';
        $files = Storage::disk('public')->files($certificatePath);
    ...
    private function optimizeImage($filePath, $convertToWebp = false)
    {
        ...
        if ($saved) {
            ...
            if ($convertToWebp && function_exists('imagewebp')) {
                ...
                $this->updateCertificateImageInDatabase($filePath, $webpPath);
            }
    ...
    private function updateCertificateImageInDatabase($originalPath, $webpPath)
    {
        try {
            // Find certificate with this image (try both original and WebP paths)
            $certificate = Certificate::where('image', $originalPath)
                ->orWhere('image', $webpPath)
                ->first();
            
            if ($certificate) {
                // Check if already using WebP
                if ($certificate->image === $webpPath) {
                    $this->line("Certificate ID {$certificate->id} already uses WebP image: {$webpPath}");
                } else {
                    // Update to WebP version
                    $certificate->update(['image' => $webpPath]);
                    $this->line("Database updated: Certificate ID {$certificate->id} now uses WebP image");
                ...
                $this->warn("No certificate found with image: {$originalPath}");
            }
        } catch (\Exception $e) {
