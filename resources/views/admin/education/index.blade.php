...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Təhsil Siyahısı</h6>
        </div>
        <div class="card-body">
                        ...
                        <tbody id="sortableEducation">
                            @foreach($education as $item)
                                <tr data-id="{{ $item->id }}">
                                    ...
                                    <td>
                                        <div class="fw-bold text-primary">{{ $item->getUniversityName() }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            ...
                                            <form action="{{ route('admin.education.destroy', $item) }}" 
                                                  method="POST" 
                                                  class="d-inline" 
                                                  onsubmit="return confirm('Bu təhsili silmək istədiyinizə əminsiniz?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
            ...
            @else
                <div class="text-center py-4">
                    ...
                    <h5 class="text-muted">Hələ heç bir təhsil əlavə edilməyib</h5>
                    <p class="text-muted">İlk təhsili əlavə etmək üçün "Yeni Təhsil" düyməsinə basın</p>
                    <a href="{{ route('admin.education.create') }}" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Yeni Təhsil
                    </a>
                </div>
...
$(document).ready(function() {
    // Initialize Sortable
    var sortable = Sortable.create(document.getElementById('sortableEducation'), {
        handle: '.reorder-handle',
        animation: 150,
        onEnd: function(evt) {
            var items = [];
            $('#sortableEducation tr').each(function(index) {
                items.push({
                    id: $(this).data('id'),
                    order: index + 1
                });
            ...
            $.ajax({
                url: '{{ route("admin.education.reorder") }}',
                method: 'POST',
                data: {
                    orders: items,
                    _token: '{{ csrf_token() }}'
