...
@section('title', 'Təcrübə Redaktə Et')
...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h1 class="h3 mb-0 text-gray-800">
                <i class="fas fa-edit"></i> Təcrübə Redaktə Et
            </h1>
        </div>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Təcrübə Məlumatları</h6>
        </div>
        <div class="card-body">
                ...
                <div class="tab-content" id="languageTabContent">
                    @foreach($languages as $language)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" 
                            ...
                            <div class="mb-3">
                                <label class="form-label">Pozisiya ({{ $language->title }}) <span class="text-danger">*</span></label>
                                <input type="text" 
                                       name="title[{{ $language->lang_code }}]" 
                                       class="form-control @error('title.' . $language->lang_code) is-invalid @enderror"
                                       value="{{ old('title.' . $language->lang_code, $experience->getTitle($language->lang_code)) }}"
                                       placeholder="Pozisiya adını daxil edin..."
                                       required>
                            ...
                            <div class="mb-3">
                                <label class="form-label">Şirkət Adı ({{ $language->title }}) <span class="text-danger">*</span></label>
                                <input type="text" 
                                       name="company_name[{{ $language->lang_code }}]" 
                                       class="form-control @error('company_name.' . $language->lang_code) is-invalid @enderror"
                                       value="{{ old('company_name.' . $language->lang_code, $experience->getCompanyName($language->lang_code)) }}"
                                       placeholder="Şirkət adını daxil edin..."
                                       required>
                                @error('company_name.' . $language->lang_code)
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description[{{ $language->lang_code }}]" 
                                          ...
                                          rows="4"
                                          placeholder="Təcrübə təsvirini daxil edin..."
                                          required>{{ old('description.' . $language->lang_code, $experience->getDescription($language->lang_code)) }}</textarea>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ $experience->status ? 'checked' : '' }}>
